...
class ActionDelegationPolicyPersistenceTests(ServicePersistenceTestBase):
    def _build_approved_shuffle_delegation_context(
        self,
        *,
        approval_decision_id: str,
        action_request_id: str,
        idempotency_key: str,
    ) -> tuple[object, AegisOpsControlPlaneService, dict[str, object], datetime]:
        store, _ = make_store()
    ...
    def test_service_serializes_action_execution_idempotency_claim(
        ...
        store, service, approved_payload, delegated_at = (
            self._build_approved_shuffle_delegation_context(
                approval_decision_id="approval-routine-serializable-001",
                action_request_id="action-request-routine-serializable-001",
                idempotency_key="idempotency-routine-serializable-001",
            )
        )
    ...
    def test_service_treats_delegated_event_emission_as_best_effort(
        self,
    ) -> None:
        store, service, approved_payload, delegated_at = (
            self._build_approved_shuffle_delegation_context(
                approval_decision_id="approval-routine-event-failure-001",
                action_request_id="action-request-routine-event-failure-001",
                idempotency_key="idempotency-routine-event-failure-001",
            )
        )
        ...
        with (
            mock.patch.object(
                service,
                "_emit_action_execution_delegated_event",
                side_effect=RuntimeError("synthetic delegated event failure"),
            ),
            mock.patch.object(service._logger, "exception") as logger_exception,
        ):
            execution = service.delegate_approved_action_to_shuffle(
                action_request_id="action-request-routine-event-failure-001",
                approved_payload=approved_payload,
        ...
        self.assertEqual(execution.lifecycle_state, "queued")
        self.assertEqual(
            store.get(ActionExecutionRecord, execution.action_execution_id),
            execution,
        )
    ...
    def test_service_preserves_finalization_failure_recording_error_context(
        ...
        service.persist_record(
            ApprovalDecisionRecord(
                approval_decision_id="approval-routine-recording-failure-001",
                action_request_id="action-request-routine-recording-failure-001",
                approver_identities=("approver-001",),
                target_snapshot=approved_target_scope,
                payload_hash=payload_hash,
                decided_at=requested_at,
        ...
        service.persist_record(
            ActionRequestRecord(
                ...
                alert_id="alert-001",
                finding_id="finding-001",
                idempotency_key="idempotency-routine-recording-failure-001",
                target_scope=approved_target_scope,
                payload_hash=payload_hash,
                requested_at=requested_at,
                expires_at=expires_at,
        ...
        recording_error = RuntimeError(
            "synthetic failure metadata recording failure"
        )
        with (
            mock.patch.object(
                service._execution_coordinator._delegation,
                "_record_dispatch_finalization_failure",
        ...
        self.assertIs(raised.exception.__cause__, recording_error)
        logger_exception.assert_called_once_with(
            "failed to persist dispatch finalization failure metadata",
            extra={
                "action_execution_id": mock.ANY,
                "execution_run_id": mock.ANY,
            },
        )
