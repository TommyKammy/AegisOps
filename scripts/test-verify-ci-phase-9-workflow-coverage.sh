...
self_guard_step_name="Run Phase 9 workflow coverage guard"
self_guard_command="bash scripts/test-verify-ci-phase-9-workflow-coverage.sh"
...
for command in "${required_verifiers[@]}"; do
  if ! grep -Fqx -- "${command}" <<<"${active_run_commands}" >/dev/null; then
    echo "Missing Phase 9 verifier command in CI workflow: ${command}" >&2
    exit 1
  fi
done
...
for command in "${required_tests[@]}"; do
  if ! grep -Fqx -- "${command}" <<<"${active_run_commands}" >/dev/null; then
    echo "Missing Phase 9 focused shell test command in CI workflow: ${command}" >&2
    exit 1
  fi
done
...
for command in "${required_runtime_commands[@]}"; do
  if ! grep -Fqx -- "${command}" <<<"${active_run_commands}" >/dev/null; then
    echo "Missing Phase 9 control-plane runtime test command in CI workflow: ${command}" >&2
    exit 1
  fi
done

if ! grep -Eq "^[[:space:]]*- name: ${self_guard_step_name}\$" "${workflow_path}"; then
  echo "Missing dedicated Phase 9 workflow coverage guard step in CI workflow: ${self_guard_step_name}" >&2
  exit 1
fi

self_guard_count="$(grep -Fxc -- "${self_guard_command}" <<<"${active_run_commands}" || true)"
if [[ "${self_guard_count}" -lt 2 ]]; then
  echo "Phase 9 workflow coverage checker must run both as a dedicated guard and within focused shell tests." >&2
  exit 1
fi

echo "CI workflow includes the required Phase 9 verifier, focused shell test, and control-plane runtime test commands."
