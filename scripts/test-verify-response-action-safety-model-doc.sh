...
ambiguous_request_identifier_repo="${workdir}/ambiguous-request-identifier"
create_repo "${ambiguous_request_identifier_repo}"
write_doc "${ambiguous_request_identifier_repo}" '# AegisOps Response Action Safety and Approval Binding Model
...
This model preserves the baseline separation between detection, approval, and execution and prevents approval from degrading into a generic approve button.'
commit_fixture "${ambiguous_request_identifier_repo}"
assert_fails_with "${ambiguous_request_identifier_repo}" "Ambiguous response action safety model statement: | Request identifier | Distinguishes one requested action from later edits, retries, or related cases. |"
