...
required_adapter_phrases=(
  ...
  'enrichment response {field_name} must be {algorithm} hex'
...
required_test_phrases=(
  ...
  'test_unavailable_source_rejects_response_body'
  ...
  'test_source_native_malwarebazaar_data_hashes_are_accepted'
  'test_response_hash_fields_must_match_declared_algorithm'
...
required_doc_phrases=(
  ...
  'Source-native `get_info` payloads may carry sample records under a top-level `data` array.'
  'It rejects malformed reviewed hashes, available responses without `query_status: ok`, responses where none of the returned hashes match the reviewed file hash, declared hash fields whose value length does not match their algorithm, unavailable states that include a response body, and custody `response_digest` values that do not match the canonical JSON digest of the response.'
...
required_validation_phrases=(
  ...
  'The focused adapter test suite accepts normal reviewed MalwareBazaar hash reputation results, including MD5-reviewed requests that match a later returned response hash and source-native `get_info` payloads with sample records under `data`, and rejects or degrades the requested boundary cases: stale reputation, unavailable source state without a response body, unavailable source state with a response body, conflicting enrichment, missing custody, source mismatch, malformed hash syntax, declared hash field algorithm mismatch, non-ok lookup status, response hash mismatch, response digest mismatch, and enrichment-driven approval, workflow-authority, endpoint-command, or field-name authority claims.'
