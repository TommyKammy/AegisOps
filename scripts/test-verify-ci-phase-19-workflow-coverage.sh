...
self_guard_command="bash scripts/test-verify-ci-phase-19-workflow-coverage.sh"
...
for command in "${required_verifiers[@]}"; do
  if ! grep -Fqx -- "${command}" <<<"${active_run_commands}" >/dev/null; then
    echo "Missing Phase 19 verifier command in CI workflow: ${command}" >&2
    exit 1
  fi
done
...
for command in "${required_tests[@]}"; do
  if ! grep -Fqx -- "${command}" <<<"${active_run_commands}" >/dev/null; then
    echo "Missing Phase 19 focused shell test command in CI workflow: ${command}" >&2
    exit 1
  fi
done
...
for command in "${required_runtime_commands[@]}"; do
  if ! grep -Fqx -- "${command}" <<<"${active_run_commands}" >/dev/null; then
    echo "Missing Phase 19 focused runtime command in CI workflow: ${command}" >&2
    exit 1
  fi
done
...
if [[ -z "${self_guard_step_run_command}" ]]; then
  echo "Missing dedicated Phase 19 workflow coverage guard step in CI workflow: ${self_guard_step_name}" >&2
  exit 1
fi

if [[ "${self_guard_step_run_command}" != "${self_guard_command}" ]]; then
  echo "Dedicated Phase 19 workflow coverage guard step must run exactly: ${self_guard_command}" >&2
...
if [[ "${self_guard_count}" -lt 2 ]]; then
  echo "Phase 19 workflow coverage checker must run both as a dedicated guard and within focused shell tests." >&2
  exit 1
fi

echo "CI workflow includes the required Phase 19 verifier, focused shell test, and focused runtime command."
