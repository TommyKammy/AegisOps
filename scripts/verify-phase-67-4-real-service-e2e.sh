...
reject_fixed "${e2e_root}/run_real_journey.py" 'observed_at=datetime.now(timezone.utc)'
reject_fixed "${lab_root}/shuffle/run_real_trial.py" 'observed_at=datetime.now(timezone.utc)'
require_fixed \
  "${repo_root}/control-plane/aegisops/control_plane/adapters/shuffle_real.py" \
...
require_fixed "${e2e_root}/build_evidence.py" '_component_observation_times'
require_fixed "${e2e_root}/build_evidence.py" 'Wazuh negative probes must follow the snapshotted health check'
require_fixed "${e2e_root}/build_evidence.py" 'Wazuh replay must follow admission and precede case promotion'
require_fixed "${e2e_root}/build_evidence.py" '_validate_wazuh_finding_identity'
...
require_fixed "${e2e_root}/validate_evidence_manifest.py" 'must retain observed Shuffle action and worker '
require_fixed "${e2e_root}/validate_evidence_manifest.py" 'CURRENT_RUNTIME_IMAGE_ID_SERVICES'
...
require_fixed "${e2e_root}/validate_evidence_manifest.py" '_schema_ordered_step_journey'
require_fixed "${e2e_root}/validate_evidence_manifest.py" 'schema current step journey contract drifted'
require_fixed "${e2e_root}/validate_evidence_manifest.py" 'schema historical step journey contract drifted'
require_fixed \
...
require_fixed "${schema}" '"current_step_journey"'
require_fixed "${schema}" '"legacy_blocked_step_journey"'
require_fixed "${schema}" '"wazuh_rule_id": {"enum": ["5710", null]}'
require_fixed "${schema}" '"runtime_image_id"'
require_fixed "${schema}" '"const": "shuffle-action-image"'
require_fixed "${schema}" '"const": "shuffle-worker-image"'
...
require_fixed "${e2e_root}/validate_evidence_manifest.py" 'Wazuh finding ID is not bound to the reviewed rule, agent,'
require_fixed "${e2e_root}/validate_evidence_manifest.py" 'wazuh_replay_observed_at'
require_fixed "${e2e_root}/validate_evidence_manifest.py" 'shuffle_receipt_replay_observed_at'
require_fixed "${e2e_root}/validate_evidence_manifest.py" 'Wazuh ingress negative cases must share their authoritative'
require_fixed \
  "${e2e_root}/validate_evidence_manifest.py" \
...
require_fixed "${e2e_root}/build_evidence.py" 'compose_ps_json'
require_fixed "${e2e_root}/build_evidence.py" 'Compose service inventory does not match the snapshot'
require_fixed "${e2e_root}/build_evidence.py" '_validate_wazuh_manifest_contract'
require_fixed "${e2e_root}/build_evidence.py" 'retained Wazuh manifest is invalid'
require_fixed "${e2e_root}/build_evidence.py" '_validate_preserved_workflow_contract'
require_fixed "${e2e_root}/build_evidence.py" 'WAZUH_SCHEMA_PATH = LAB_ROOT / "wazuh"'
require_fixed "${e2e_root}/build_evidence.py" 'WAZUH_VALIDATOR_PATH = LAB_ROOT / "wazuh"'
require_fixed "${e2e_root}/build_evidence.py" 'WORKFLOW_VALIDATOR_PATH = ('
...
require_fixed "${schema}" '"const": "passed"'
require_fixed "${schema}" '"wazuh_replay_observed_at"'
require_fixed "${schema}" '"shuffle_receipt_replay_observed_at"'
require_fixed "${schema}" '"wazuh-manifest:duplicate_delivery"'
require_fixed "${schema}" '"ga_accepted": {'
require_fixed "${schema}" '"const": false'
...
require_fixed "${lab_root}/run-e2e-trial.sh" 'docker_lab service inspect "${shuffle_worker_service}"'
require_fixed "${lab_root}/run-e2e-trial.sh" '.Status.State == "running"'
require_fixed "${lab_root}/run-e2e-trial.sh" '.Status.ContainerStatus.ContainerID'
require_fixed "${lab_root}/run-e2e-trial.sh" '.State.Running == true'
require_fixed "${lab_root}/run-e2e-trial.sh" 'docker_lab logs'
require_fixed "${lab_root}/run-e2e-trial.sh" 'Shuffle worker container logs do not contain the reviewed execution ID'
require_fixed "${lab_root}/run-e2e-trial.sh" 'postdispatch_shuffle_worker_image'
require_fixed "${lab_root}/run-e2e-trial.sh" 'configured_shuffle_worker_immutable_ref'
...
reject_fixed "${lab_root}/run-e2e-trial.sh" '--wazuh-schema'
reject_fixed "${lab_root}/run-e2e-trial.sh" '--wazuh-validator'
reject_fixed "${lab_root}/run-e2e-trial.sh" '--workflow-validator'
...
require_fixed "${lab_root}/RUNBOOK.md" 'ownership labels, and image match'
require_fixed "${lab_root}/RUNBOOK.md" 'Wazuh replay time between admission'
