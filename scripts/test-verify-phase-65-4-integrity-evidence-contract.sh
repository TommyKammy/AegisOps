...
empty_tree="$(git -C "${artifact_absent_at_revision_repo}" mktree </dev/null)"
artifact_absent_revision="$(printf '%s\n' "artifact path gap" | git -C "${artifact_absent_at_revision_repo}" commit-tree "${empty_tree}")"
ARTIFACT_ABSENT_REVISION="${artifact_absent_revision}" perl -0pi -e 's/[0-9a-f]{40}/$ENV{ARTIFACT_ABSENT_REVISION}/g' "${artifact_absent_at_revision_repo}/docs/deployment/release/phase-65-4-integrity-evidence.yaml"
assert_fails_with "${artifact_absent_at_revision_repo}" "artifact_path must exist at repository_revision for release-channel-upgrade-manifest"
...
anchored_external_uri_reference_repo="${workdir}/anchored-external-uri-reference"
create_valid_repo "${anchored_external_uri_reference_repo}"
perl -0pi -e 's#<evidence-dir>/offline-install-bundle\.sbom\.cdx\.json#&bad s3://bucket/offline-install-bundle.sbom.cdx.json#m' "${anchored_external_uri_reference_repo}/docs/deployment/release/phase-65-4-integrity-evidence.yaml"
assert_fails_with "${anchored_external_uri_reference_repo}" "Invalid Phase 65.4 integrity manifest artifact reference"
...
wrong_kind_checksum_reference_repo="${workdir}/wrong-kind-checksum-reference"
create_valid_repo "${wrong_kind_checksum_reference_repo}"
perl -0pi -e 's#<evidence-dir>/offline-install-bundle\.sha256#<evidence-dir>/offline-install-bundle.sbom.cdx.json#m' "${wrong_kind_checksum_reference_repo}/docs/deployment/release/phase-65-4-integrity-evidence.yaml"
assert_fails_with "${wrong_kind_checksum_reference_repo}" "Invalid Phase 65.4 integrity manifest checksum_reference for offline-install-bundle"

wrong_kind_signature_reference_repo="${workdir}/wrong-kind-signature-reference"
create_valid_repo "${wrong_kind_signature_reference_repo}"
perl -0pi -e 's#<evidence-dir>/offline-install-bundle\.sigstore-placeholder\.txt#<evidence-dir>/offline-install-bundle.sha256#m' "${wrong_kind_signature_reference_repo}/docs/deployment/release/phase-65-4-integrity-evidence.yaml"
assert_fails_with "${wrong_kind_signature_reference_repo}" "Invalid Phase 65.4 integrity manifest signature_reference for offline-install-bundle"
...
merge_key_artifact_field_repo="${workdir}/merge-key-artifact-field"
create_valid_repo "${merge_key_artifact_field_repo}"
perl -0pi -e 's#(    signing_identity: <beta-signing-identity>\n)#$1    <<: {production_signing_infrastructure: true}\n#m' "${merge_key_artifact_field_repo}/docs/deployment/release/phase-65-4-integrity-evidence.yaml"
assert_fails_with "${merge_key_artifact_field_repo}" "Invalid Phase 65.4 integrity manifest artifact field for offline-install-bundle: <<"
...
readiness_authority_verb_repo="${workdir}/readiness-authority-verb"
create_valid_repo "${readiness_authority_verb_repo}"
printf '%s\n' "Phase 65.4 integrity evidence grants Beta readiness." >>"${readiness_authority_verb_repo}/docs/phase-65-4-integrity-evidence-contract.md"
assert_fails_with "${readiness_authority_verb_repo}" "Forbidden Phase 65.4 integrity evidence claim"
...
empty_block_scalar_required_repo="${workdir}/empty-block-scalar-required"
create_valid_repo "${empty_block_scalar_required_repo}"
perl -0pi -e 's/^approval_record:.*/approval_record: |/m' "${empty_block_scalar_required_repo}/docs/deployment/release/phase-65-4-integrity-evidence.yaml"
assert_fails_with "${empty_block_scalar_required_repo}" "Missing Phase 65.4 integrity manifest value: approval_record"

null_required_repo="${workdir}/null-required"
create_valid_repo "${null_required_repo}"
perl -0pi -e 's/^approval_record:.*/approval_record: null/m' "${null_required_repo}/docs/deployment/release/phase-65-4-integrity-evidence.yaml"
assert_fails_with "${null_required_repo}" "Missing Phase 65.4 integrity manifest value: approval_record"

tilde_required_repo="${workdir}/tilde-required"
create_valid_repo "${tilde_required_repo}"
perl -0pi -e 's/^approval_record:.*/approval_record: ~/m' "${tilde_required_repo}/docs/deployment/release/phase-65-4-integrity-evidence.yaml"
assert_fails_with "${tilde_required_repo}" "Missing Phase 65.4 integrity manifest value: approval_record"
...
customer_private_doc_repo="${workdir}/customer-private-doc"
create_valid_repo "${customer_private_doc_repo}"
printf '%s\n' "Phase 65.4 integrity evidence contains customer-private data." >>"${customer_private_doc_repo}/docs/phase-65-4-integrity-evidence-contract.md"
assert_fails_with "${customer_private_doc_repo}" "customer-private data"
