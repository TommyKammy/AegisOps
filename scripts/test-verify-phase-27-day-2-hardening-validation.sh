...
assert_fails_with "${missing_drill_repo}" "Missing required line in ${missing_drill_repo}/docs/phase-27-day-2-hardening-validation.md: | Restore reconciliation truth integrity | \`test_phase27_restore_reconciliation_truth_integrity_keeps_mismatch_reviewable\` proves restored subordinate receipts and external-ticket evidence cannot auto-close cases or auto-advance action execution state when the authoritative reconciliation record remains mismatched. | \`test_service_fail_closes_when_create_tracking_ticket_reconciliation_receipt_drifts\`; \`test_service_phase21_restore_rejects_reconciliation_run_binding_mismatch\` | \`python3 -m unittest control-plane.tests.test_phase27_day2_runtime_contract\` |"
