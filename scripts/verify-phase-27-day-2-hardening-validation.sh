...
require_fixed_line "${validation_doc}" "| Rollback posture | None. Phase 27 currently treats rollback as a documented operational guardrail rather than a dedicated runtime-enforced contract. | \`docs/runbook.md\`; \`docs/smb-footprint-and-deployment-profile-baseline.md\` | \`bash scripts/verify-phase-27-day-2-hardening-validation.sh\` |"
