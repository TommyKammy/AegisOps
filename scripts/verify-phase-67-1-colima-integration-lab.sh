...
require_fixed_string "${common}" 'DNS:${escaped_identity}([,[:space:]]|$)'
require_fixed_string "${common}" '*,CN=admin,*)'
...
require_fixed_string "${lab_dir}/control-plane-entrypoint.sh" "'lifecycle_transition_records_previous_lifecycle_state_known_val'"
require_fixed_string "${lab_dir}/control-plane-entrypoint.sh" 'constraint_record.contype::text'
require_fixed_string "${lab_dir}/control-plane-entrypoint.sh" 'constraint_definition_sha256'
require_fixed_string "${lab_dir}/control-plane-entrypoint.sh" 'column_definition_sha256'
require_fixed_string "${lab_dir}/control-plane-entrypoint.sh" "COALESCE(column_default, '<NULL>')"
require_fixed_string "${lab_dir}/control-plane-entrypoint.sh" 'sha256('
require_fixed_string "${lab_dir}/control-plane-entrypoint.sh" 'convert_to('
require_fixed_string "${lab_dir}/control-plane-entrypoint.sh" 'e2595161f20c3faf32d1891f26f6c8eeeb08c40c73a6db8ebb495bdec6fb7bba'
require_fixed_string "${lab_dir}/control-plane-entrypoint.sh" 'c0eecfc71b9e55e2ee8d2712cbee1448497f847711f67a4f74977155de559920'
require_fixed_string "${lab_dir}/control-plane-entrypoint.sh" 'f88e5cdaaa4b0079bfcadd42c335b0b22722fd4f856ed1e1a609b4fdf6589f6f'
...
require_fixed_string "${preflight}" 'binfmt_handler_enabled'
require_fixed_string "${preflight}" 'grep -qx enabled /proc/sys/fs/binfmt_misc/status'
require_fixed_string "${preflight}" 'grep -qx enabled "${handler_path}"'
require_fixed_string "${preflight}" 'binfmt_handler_enabled qemu-aarch64'
require_fixed_string "${preflight}" 'binfmt_handler_enabled qemu-x86_64'
require_fixed_string "${preflight}" 'test -x /mnt/lima-rosetta/rosetta'
...
require_fixed_string "${prepare}" 'failed validity, identity, chain, or key-pair validation'
...
require_fixed_string "${prepare}" '"${LAB_DIR}/preflight.sh" --scope wazuh'
...
require_fixed_string "${lab_dir}/up.sh" 'proxy-config.sha256'
require_fixed_string "${lab_dir}/up.sh" 'cat "${LAB_DIR}/config/nginx.conf"'
require_fixed_string "${lab_dir}/up.sh" 'cat "${LAB_DIR}/config/control-plane.conf"'
require_fixed_string "${lab_dir}/up.sh" 'mv "${proxy_config_state_staging}" "${proxy_config_state}"'
...
require_fixed_string "${lab_dir}/status.sh" 'control_plane_container_image_id'
require_fixed_string "${lab_dir}/status.sh" 'compose_scope "${scope}" ps --all --quiet control-plane'
require_fixed_string "${lab_dir}/status.sh" 'compose_scope "${scope}" ps --all --format json'
require_fixed_string "${lab_dir}/status.sh" 'compose_ps_json=%s'
require_fixed_string "${lab_dir}/status.sh" 'sort_by(.Service)'
...
require_fixed_string "${lab_dir}/logs.sh" 'log_tail="${AEGISOPS_LAB_LOG_TAIL:-200}"'
require_fixed_string "${lab_dir}/logs.sh" '^[1-9][0-9]{0,4}$'
require_adjacent_lines \
  "${lab_dir}/logs.sh" \
  'if [[ ! "${log_tail}" =~ ^[1-9][0-9]{0,4}$ ]] ||' \
  '  (( 10#${log_tail} > 10000 )); then'
require_fixed_string "${lab_dir}/logs.sh" 'must be an integer from 1 through 10000'
require_fixed_string "${lab_dir}/logs.sh" '--tail "${log_tail}"'
require_fixed_string "${lab_dir}/smoke-core.sh" 'for endpoint in healthz readyz runtime; do'
require_fixed_string "${init}" 'proxy_runtime_auth="${cert_dir}/runtime-auth.conf"'
require_fixed_string "${init}" 'proxy_set_header X-AegisOps-Authenticated-Identity "phase67-lab-platform-admin";'
require_fixed_string "${init}" 'proxy_set_header X-AegisOps-Authenticated-Role "platform_admin";'
require_fixed_string "${init}" 'chmod 600 "${proxy_runtime_auth}"'
require_fixed_string "${init}" 'mv "${proxy_runtime_auth_staging}" "${proxy_runtime_auth}"'
...
require_fixed_string "${readme}" 'immutable image ID used by the running control-plane container'
...
require_fixed_string "${runbook}" 'validates cached certificate expiry, reviewed leaf identities, chains, and key pairs'
...
require_fixed_string "${runbook}" 'non-production smoke identity'
require_fixed_string "${runbook}" 'immutable image ID of the actual control-plane container'
