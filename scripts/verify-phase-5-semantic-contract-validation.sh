...
bash "${repo_root}/scripts/verify-auth-baseline-doc.sh" "${repo_root}"
bash "${repo_root}/scripts/verify-retention-baseline-doc.sh" "${repo_root}"
bash "${repo_root}/scripts/verify-source-onboarding-contract-doc.sh" "${repo_root}"
bash "${repo_root}/scripts/verify-sigma-to-opensearch-translation-strategy-doc.sh" "${repo_root}"
...
required_phrases=(
  ...
  "- Validation date:"
  "- Validation scope: Telemetry schema, SecOps domain semantics, detection lifecycle, approval binding, control-plane state, operating model, identity boundaries, retention and replay readiness, source onboarding, and Sigma translation boundary"
