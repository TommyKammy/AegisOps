...
contract_path="${repo_root}/docs/deployment/shuffle-notify-identity-owner-template-import-contract.md"
artifact_path="${repo_root}/docs/deployment/profiles/smb-single-node/shuffle/templates/notify_identity_owner-import-contract.yaml"
readme_path="${repo_root}/README.md"
expected_authority_boundary="Shuffle workflow status, success, failure, callback payloads, execution logs, generated config, and template metadata remain subordinate automation context; AegisOps control-plane records remain authoritative for approval, action request, execution receipt, reconciliation, release, gate, limitation, and closeout truth."
...
required_phrases=(
  ...
  "This contract defines the repo-owned \`notify_identity_owner\` Shuffle workflow template import contract for the Phase 54 \`smb-single-node\` product profile."
  ...
  "The contract binds request identity, approval identity, correlation identity, receipt identity, recipient identity owner, message scope, and reviewed template version without treating Shuffle state as authoritative AegisOps truth."
...
assert_top_field_equals() {
  ...
  if [[ "${actual}" != "${expected}" ]]; then
    ...
    echo "Mismatched Phase 54.3 notify_identity_owner import artifact field ${field}: expected [${expected}] actual [${actual}]" >&2
    exit 1
  fi
}
...
assert_section_field_equals() {
  ...
  if [[ "${actual}" != "${expected}" ]]; then
    [[ -n "${actual}" ]] || actual="<missing>"
    echo "Mismatched Phase 54.3 notify_identity_owner import artifact field ${section}.${field}: expected [${expected}] actual [${actual}]" >&2
    exit 1
  fi
}
...
assert_list_contains() {
  ...
  if ! awk -v list_name="${list_name}" -v item="${item}" '
    ...
    END { exit(found ? 0 : 1) }
  ' "${artifact_path}"; then
    echo "Missing Phase 54.3 notify_identity_owner import artifact ${list_name} item: ${item}" >&2
    exit 1
  fi
}
...
if [[ ! -f "${contract_path}" ]]; then
  echo "Missing Phase 54.3 notify_identity_owner template import contract: ${contract_path}" >&2
  exit 1
fi

if [[ ! -f "${artifact_path}" ]]; then
  echo "Missing Phase 54.3 notify_identity_owner template import artifact: ${artifact_path}" >&2
  exit 1
fi
...
for heading in "${required_headings[@]}"; do
  if ! grep -Fxq -- "${heading}" <<<"${contract_rendered_markdown}"; then
    echo "Missing Phase 54.3 notify_identity_owner template import contract heading: ${heading}" >&2
    exit 1
  fi
done
...
for phrase in "${required_phrases[@]}"; do
  if ! grep -Fq -- "${phrase}" <<<"${contract_rendered_markdown}"; then
    echo "Missing Phase 54.3 notify_identity_owner template import contract statement: ${phrase}" >&2
    exit 1
  fi
done
...
for line in "${required_artifact_lines[@]}"; do
  if ! grep -Fxq -- "${line}" "${artifact_path}"; then
    echo "Missing exact Phase 54.3 notify_identity_owner import artifact line: ${line}" >&2
    exit 1
  fi
done

assert_top_field_equals "contract_id" "shuffle-notify-identity-owner-template-import"
assert_top_field_equals "product_profile" "shuffle"
assert_top_field_equals "profile_id" "smb-single-node"
assert_top_field_equals "workflow_template_id" "notify_identity_owner"
...
assert_section_field_equals "template_metadata" "template_id" "notify_identity_owner"
assert_section_field_equals "template_metadata" "reviewed_template_version" "notify_identity_owner-v1-reviewed-2026-05-03"
...
assert_section_field_equals "template_metadata" "action_type" "notify_identity_owner"
assert_section_field_equals "binding_contract" "message_scope" "identity-owner-low-risk-notification-only"
assert_section_field_equals "binding_contract" "reviewed_template_version" "notify_identity_owner-v1-reviewed-2026-05-03"
assert_section_field_equals "validation_rules" "review_status" "reviewed"
assert_section_field_equals "validation_rules" "action_type" "notify_identity_owner"
...
assert_section_field_equals "validation_rules" "missing_recipient_identity_owner" "reject"
assert_section_field_equals "validation_rules" "missing_message_scope" "reject"
...
assert_section_field_equals "mutation_boundary" "notification_only" "required"
assert_section_field_equals "mutation_boundary" "allowed_target_effect" "outbound notification to recipient identity owner only"
...
for claim in "${forbidden_claims[@]}"; do
  if contains_forbidden_outside_forbidden_section "${claim}"; then
    echo "Forbidden Phase 54.3 notify_identity_owner template import contract claim: ${claim}" >&2
    exit 1
  fi
done

if contains_placeholder_secret_valid_claim; then
  echo "Forbidden Phase 54.3 notify_identity_owner template import contract claim: placeholder secrets accepted as valid credentials" >&2
  exit 1
fi

if grep -Eq 'review_status:[[:space:]]*(unreviewed|draft|sample|placeholder|TODO|todo|deprecated)' "${artifact_path}"; then
  echo "Forbidden Phase 54.3 notify_identity_owner import artifact: unreviewed template status enters product mainline" >&2
  exit 1
fi
...
if grep -Eq "(${path_token_boundary}${local_path_token}|file:///?${local_path_token})" "${contract_path}" "${artifact_path}"; then
  echo "Forbidden Phase 54.3 notify_identity_owner template import contract: workstation-local absolute path detected" >&2
  exit 1
fi

if [[ ! -f "${readme_path}" ]]; then
  echo "Missing README for Phase 54.3 notify_identity_owner template import contract link check: ${readme_path}" >&2
  exit 1
fi
...
if ! grep -Eq '\[[^]]+\]\(docs/deployment/shuffle-notify-identity-owner-template-import-contract\.md\)' <<<"${readme_rendered_markdown}"; then
  echo "README must link the Phase 54.3 notify_identity_owner template import contract." >&2
  exit 1
fi

echo "Phase 54.3 notify_identity_owner template import contract is present and preserves required request, approval, correlation, receipt, recipient identity owner, message scope, reviewed version, and protected-state mutation boundaries."
