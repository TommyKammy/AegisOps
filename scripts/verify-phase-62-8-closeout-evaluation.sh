...
if claim_scan_text "${absolute_doc_path}" | awk -v allowed_non_claim_line="${allowed_non_claim_line_lower}" \
  -v required_rejection_line="${required_rejection_line_lower}" '
  {
    ...
    if (line ~ /(^|[^[:alnum:]_])(ui cache|ui state|ui surface|browser state|browser cache|ticket state|ticket status|downstream ticket|simulator output|downstream workflow|shuffle workflow)([[:space:]]+(state|status|output|cache|surface))?[[:space:]]+(has|have|had)([[:space:]]+[[:alpha:]-]+){0,3}[[:space:]]+(approved|authorized|executed|reconciled|closed|gated|validated)([^[:alnum:]_]|$|[[:space:]])/) {
      found_kind = "subordinate-surface authority overclaim"
    }
  }
  END {
