...
- [Phase 54.1 Shuffle profile contract](docs/deployment/shuffle-smb-single-node-profile-contract.md) defines the `smb-single-node` Shuffle frontend, backend, orborus, worker, OpenSearch, API/callback, credential, volume, port, and pinned-version expectations.
- [Phase 54.2 reviewed workflow template contract](docs/deployment/shuffle-reviewed-workflow-template-contract.md) defines the required Shuffle template metadata, input, output, correlation, action request, approval decision, execution receipt, version, owner, and review-status fields before import.
- [Phase 54.3 notify_identity_owner template import contract](docs/deployment/shuffle-notify-identity-owner-template-import-contract.md) defines the reviewed low-risk notification template import contract, including request, approval, correlation, receipt, recipient identity owner, message scope, reviewed-version, and protected-state mutation boundaries.
...
- [Phase 62.5 manual fallback contract](docs/phase-62-5-manual-fallback-contract.md) extends manual fallback owner, operator note, affected action, blocked reason, expected evidence, and follow-up posture requirements to every reviewed Phase 62 catalog action without creating approval, execution, or reconciliation authority.
...
- [Phase 65.1 release bundle inventory](docs/phase-65-1-release-bundle-inventory.md) defines the versioned beta/design-partner artifact inventory, owners, evidence references, exclusions, verifier coverage, and non-claims before later Phase 65 packaging slices.
- [Phase 65.2 offline install bundle contract](docs/phase-65-2-offline-install-bundle-contract.md) defines the bounded offline install bundle shape, required files, metadata, smoke validation, forbidden material, and beta/design-partner non-claims.
- [Phase 65.3 release channel and upgrade manifest contract](docs/phase-65-3-release-channel-upgrade-manifest-contract.md) defines beta/design-partner release-channel metadata and upgrade manifest evidence for compatible and incompatible version posture without silent auto-upgrade, hosted update-service, RC, or GA claims.
- [Phase 65.4 integrity evidence contract](docs/phase-65-4-integrity-evidence-contract.md) defines beta/design-partner SBOM, checksum, and signing evidence expectations with artifact identity binding and no production signing, entitlement, RC, or GA claims.
- [Phase 65.5 OSS licensing and redistribution review checklist](docs/phase-65-5-oss-licensing-redistribution-checklist.md) defines beta/design-partner licensing checklist fields, Wazuh and Shuffle packaging boundaries, blocker disposition, and non-claims without legal advice or production distribution approval.
- [Phase 65.6 default SMB documentation pack](docs/phase-65-6-default-smb-documentation-pack.md) defines beta/design-partner SMB docs pack coverage for install, daily operation, source onboarding, automation, AI usage, backup and restore, support bundle, upgrade, and rollback without RC, GA, or production-support claims.
...
- [Phase 67.1 Colima integration lab](control-plane/deployment/phase-67-integration-lab/README.md) provides the isolated non-production scaffold, explicit architecture posture, preflight, lifecycle commands, and core health proof for real Wazuh and Shuffle container trials.
- [Phase 67.4 real-service E2E prerequisite evaluation](docs/phase-67-prerequisite-evaluation.md) records the bounded Wazuh-to-AegisOps-to-Shuffle trial, machine-verifiable redacted evidence, owned limitations, and explicit GA non-acceptance.
...
The Phase 51.3 Pilot, Beta, RC, and GA gate contract is defined by the [Phase 51.3 pilot beta RC GA gate contract](docs/phase-51-3-pilot-beta-rc-ga-gate-contract.md); Phase 66 is RC, Phase 67 performs bounded GA-prerequisite validation, and GA acceptance remains a separate evidence-backed decision.
...
The Phase 66.4 AI-assisted triage RC proof is defined by the [Phase 66.4 AI-assisted triage RC proof](docs/phase-66-4-ai-assisted-triage-rc-proof.md).
...
The Phase 65.9 closeout evaluation is defined by the [Phase 65.9 closeout evaluation](docs/phase-65-closeout-evaluation.md).
