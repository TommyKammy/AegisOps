...
- What changed: Added `Reconciliation` to the control-plane ownership model and minimum record-family set, defined reconciliation identifiers and lifecycle states, reserved `reconciliation_records` in the PostgreSQL README/schema/migration skeleton, tightened the control-plane/Phase 8 verifier scripts plus focused shell tests to fail closed on omission, committed the checkpoint as `e945329`, and opened draft PR #179.
- Current blocker: none
- Next exact step: Monitor PR #179 and respond to review or CI feedback if any follow-up drift appears around the reconciliation boundary.
